...
def perform_fit2(xs, ys,variables = [1,1]):
    fit_vals, cov_vals = optimize.curve_fit(
                    lambda t,a,b: pow_func2([a,b],t),
...
def perform_fit3(xs, ys,variables = [1,1,0]):
    fit_vals, cov_vals = optimize.curve_fit(
                    lambda t,a,b,c: pow_func3([a,b,c],t),
...
def plot_scan_noise_vs_numCells():
    ...
    for i in range(num_sec_vals):
        ...
        plt.errorbar(results[i][0], results[i][1], yerr=results[i][2], marker='x', label = this_label)
...
def plot_time_vs_timeLength():
    ...
    # Fit
    fit_vals, fit_errs = perform_fit2(results[0], results[3])
    num_fit_points = 100
...
def plot_time_vs_numParts():
    ...
    # Fit
    fit_vals, fit_errs = perform_fit3(results[0], results[3])
    num_fit_points = 100
    fit_xs = numpy.logspace(start_var, stop_var, num_fit_points, base=this_log_base)
...
def plot_time_vs_length():
    ...
    # Fit
    fit_vals, fit_errs = perform_fit3(results[0], results[3], variables=[0,-1,0])
    num_fit_points = 100
    fit_xs = numpy.linspace(start_var, stop_var, num_fit_points)
    fit_ys = pow_func3(fit_vals, fit_xs)
    fit_label = r'Power Fit: $(%.2g\pm %.1g)(x)^{(%.2g\pm %.1g)}+(%.2g\pm %.1g)$' % (
