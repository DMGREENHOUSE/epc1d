...
def calc_density_adapted(np.ndarray[double, ndim=1] position, int ncells, double L):
    ...
    density_next_c = np.concatenate(([density_next_c[-1]],density_next_c[1:-1]))
    density_c += density_next_c
